import enqr from "../src/index";
import { createCanvas, loadImage, Canvas } from "canvas";

/*
QR code, EC level H, Content: http://www.google.com/
 */
const GOLDEN: string = `data:image/png;base64,`;

test("library", async () => {
    expect(typeof enqr).toBe("function");

    const qr = enqr("http://www.google.com/", {
        errorCorrection: "M"
    });

    const canvas = qr.renderToCanvas({
        targetSize: qr.matrix.width * 4,
        quietZone: 8
    }) as Canvas;

    const golden = await loadImage(GOLDEN);
    expect(canvas.width).toEqual(golden.width);
    expect(canvas.height).toEqual(golden.height);

    const goldenCanvas = createCanvas(golden.width, golden.height);
    goldenCanvas.getContext("2d")!
        .drawImage(golden, 0, 0);

    const a = canvas.toBuffer("raw");
    const b = goldenCanvas.toBuffer("raw");

    expect(a.length).toEqual(b.length);

    for (let i=0; i < a.length; i++)
        expect(a[i] / 256).toBeCloseTo(b[i] / 256);
});
